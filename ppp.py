...
class WpContent:
	def __init__(self, url):
		self.__url = url
		self.__response = urllib3.urlopen(self.__url).read()
	...
	def get_wp_post_information(self):
		get_post = urllib3.urlopen(self.get_api_wp()+"wp/v2/posts").read()
	...
	def inject_content(self, id_content, title, content):
		...
		req = urllib3.Request(full_url, data, params)
		resp = urllib3.urlopen(req).read()
		return resp
